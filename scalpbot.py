...
VERSION = "5.6-DASHBOARD-RELIABILITY"
...
</aside><main id="home"><header class="top"><div><h1>Trading Dashboard <span class="tag">V5.6</span></h1><p>SCALPBOT PRO · Hesap ve piyasa görünümü</p></div><div class="topright"><div class="pill"><span class="dot"></span><strong id="status">Bağlanıyor</strong></div><div class="pill" id="updated">Güncelleme bekleniyor</div><button type="button" class="tag" id="refreshDashboard">⟳ Tümünü yenile</button></div></header>
...
function pnl(el,n){const node=$(el);const next=money(n);if(node.textContent!==next)flash(el);node.textContent=next;node.className=(Number(n)>=0?'green':'red')}
