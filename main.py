...
@app.route('/')
def home():
    return jsonify({"status": "active", "bot": "ForexScalpBot-15m"}), 200
...
# --- SCALP İÇİN 15M VE ESNEK FİLTRE AYARI ---
TIMEFRAME = "15m"  
RISK_MIKTARI = 150.0  
LAST_UPDATE_ID = 0
                ...
                for update in res["result"]:
                    ...
                    if "message" in update and "text" in update["message"]:
                        ...
                        if txt in ["/start", "/yardim", "yardım"]:
                            telegram_gonder(
                                "🤖 *Forex/Altın 15M Scalp Simülasyon Botu*\n\n"
                                "• `/durum` - Sanal kasa ve pozisyonlar\n"
                                "• `/fiyat` - Canlı fiyatlar ve 15m RSI\n"
                            ...
                            rapor = "📊 *Canlı 15 Dakikalık Piyasalar*\n\n"
...
def piyasa_tarayici_worker():
    ...
    telegram_gonder("🚀 *Forex/Altın 15M Scalp Simülasyon Botu Başlatıldı!* ($3000 Sanal Kasa)")
    ...
    while True:
            ...
            if not haffacilik_kapali_mi():
                ...
                for sembol, isim in PARITELER.items():
                    ...
                    try:
                        ...
                        # Scalp esnek momentum koşulu (15m tetikleme)
                        ...
                        if long_kosul:
                            ...
                            kasa_kaydet(KASA)
                            telegram_gonder(
                                f"⚡ *SCALP SİNYALİ (SHORT)* | {isim} (15M)\n"
                                f"🔴 Giriş: `{fiyat}` | 🛑 SL: `{sl}` | 🎯 TP: `{tp}`\n"
                                f"📊 RSI: `{rsi}`"
                            )
                    except Exception as e:
        ...
        time.sleep(300) # 5 dakikada bir 15m tarama döngüsü
